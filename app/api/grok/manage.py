...
router = APIRouter(tags=["Grok管理"])

# 常量
# Path(__file__) is app/api/grok/manage.py
# parents[2] is app/
STATIC_DIR = Path(__file__).parents[2] / "templates" / "grok_admin"
...
async def login_page():
    ...
    raise HTTPException(status_code=404, detail=f"登录页面不存在: {login_html}")
...
async def manage_page():
    ...
    raise HTTPException(status_code=404, detail=f"管理页面不存在: {admin_html}")
...
async def admin_login(request: LoginRequest) -> LoginResponse:
    ...
    try:
        ...
        expected_pass = setting.global_config.get("admin_password", "admin")
